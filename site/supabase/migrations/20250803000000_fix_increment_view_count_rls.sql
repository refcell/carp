-- Fix increment_view_count function to bypass RLS policies
-- The issue is that even with SECURITY DEFINER, RLS policies are still being applied

-- Drop the existing function
DROP FUNCTION IF EXISTS increment_view_count(UUID);

-- Recreate the function with explicit RLS bypass
CREATE OR REPLACE FUNCTION increment_view_count(agent_id UUID)
RETURNS TABLE(new_view_count INTEGER) AS $$
BEGIN
  -- Temporarily disable RLS for this function
  -- This is safe because we're only incrementing view counts, not sensitive operations
  SET local row_security = off;
  
  -- Atomically increment and return the new count
  UPDATE public.agents 
  SET view_count = view_count + 1 
  WHERE id = agent_id;
  
  -- Return the new view count
  RETURN QUERY 
  SELECT agents.view_count 
  FROM public.agents 
  WHERE agents.id = agent_id;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- Grant execute permission to authenticated users and anonymous users
GRANT EXECUTE ON FUNCTION increment_view_count(UUID) TO authenticated;
GRANT EXECUTE ON FUNCTION increment_view_count(UUID) TO anon;

-- Add comment explaining the security model
COMMENT ON FUNCTION increment_view_count(UUID) IS 
'Atomically increments the view count for an agent. Uses SECURITY DEFINER and disables RLS to allow anonymous users to increment view counts for any public agent.';