...
impl AgentManifest {
    ...
    pub fn validate(&self) -> CarpResult<()> {
        if self.name.is_empty() {
            return Err(CarpError::ManifestError("Agent name cannot be empty".to_string()));
        }
        ...
        if self.version.is_empty() {
            return Err(CarpError::ManifestError("Version cannot be empty".to_string()));
        }
        
        // Basic semver validation
        if !self.version.split('.').all(|part| part.chars().all(|c| c.is_numeric())) {
            return Err(CarpError::ManifestError(
        ...
        if self.description.is_empty() {
            return Err(CarpError::ManifestError("Description cannot be empty".to_string()));
        }
        
        if self.author.is_empty() {
            return Err(CarpError::ManifestError("Author cannot be empty".to_string()));
        }
