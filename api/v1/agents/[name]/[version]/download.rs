    ...
    match get_agent_download_info(&agent_name, &version, &req).await {
        ...
        Err(e) => {
            let error = ApiError {
                ...
                message: format!("Agent '{}' version '{}' not found: {}", agent_name, version, e),
                details: None,
            };
            Ok(Response::builder()
                .status(404)
...
async fn get_agent_download_info(name: &str, version: &str, req: &Request) -> AnyhowResult<AgentDownload> {
    ...
    let download_url = generate_signed_url(&client, &supabase_url, &supabase_key, &agent_info.file_path).await?;
...
async fn query_agent_info(
    ...
    if let Some(data) = result.as_array().and_then(|arr| arr.first()) {
        ...
        Err(anyhow!("Agent not found or no valid response from database"))
    }
}
...
async fn generate_signed_url(
    ...
    let url = format!("{}/storage/v1/object/sign/agent-packages/{}", supabase_url, file_path);
