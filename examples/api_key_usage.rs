...
impl ApiKeyManager {
    ...
    pub async fn verify_api_key(
        ...
        let mut response = self.client
            .rpc("verify_api_key", format!(r#"{{"key_hash_param": "{}"}}"#, key_hash))
            .execute()
            .await;
        ...
        if response.is_err() || !response.as_ref().unwrap().status().is_success() {
            response = self.client
                .rpc("validate_api_key", format!(r#"{{"api_key_hash": "{}"}}"#, key_hash))
                .execute()
                .await;
        }
    ...
    async fn update_last_used(&self, key_hash: &str) -> Result<(), Box<dyn std::error::Error>> {
        let _ = self.client
            .rpc("update_api_key_last_used", format!(r#"{{"key_hash_param": "{}"}}"#, key_hash))
            .execute()
            .await?;
