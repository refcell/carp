...
async fn demonstrate_complete_flow() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (endpoint, description, required_scope) in jwt_endpoints {
        ...
        println!(
            "     {} {} - {}: {}",
            status, endpoint, description, required_scope
        );
    }
    ...
    let api_key_endpoints = vec![
        ...
        (
            "GET /v1/agents/my-agent/1.0.0/download",
            "Download agent",
            "read",
        ),
...
async fn demonstrate_error_scenarios() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "   ❌ API key '{}' used on JWT-only endpoint /v1/auth/api-keys POST",
        &api_key[..20]
    );
    ...
    for (required_scope, operation) in scope_tests {
        ...
        if !has_scope {
            println!(
                "   ❌ User with scopes {:?} cannot {}",
                limited_user_scopes, operation
            );
            println!(
                "      Expected error: 'Required scope {} not found'",
                required_scope
            );
        }
    }
...
async fn demonstrate_configuration_scenarios() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let env_vars = vec![
        ...
        (
            ...
            env::var("SUPABASE_SERVICE_ROLE_KEY").unwrap_or_default(),
        ),
    ];
...
mod integration_tests {
    ...
    fn test_token_format_validation() {
        ...
        let is_valid_api_key =
            valid_api_key.starts_with("carp_") && valid_api_key.matches('_').count() == 3;
        assert!(is_valid_api_key);

        // Test invalid API key format
        let invalid_api_key = "carp_incomplete";
        let is_invalid_api_key =
            invalid_api_key.starts_with("carp_") && invalid_api_key.matches('_').count() == 3;
